...
def format_messages(db: Session, all_messages: list[Message]) -> list[MessageInDB]:
    ...
    for m in all_messages:
        sender_email = db.query(User).filter(User.id == m.sender_id).first().email
...
def get_all_unseen(new_message: NewMessage, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if receiver is None:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "No such email registered")
    sender_id = db.query(User).filter(User.email == current_user.email).first().id
    to_add = Message(content=new_message.content, sender_id=sender_id, receiver_id=receiver.id)
    db.add(to_add)
    db.commit()
    return new_message 
